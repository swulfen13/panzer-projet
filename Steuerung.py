...
async def control_hub_1(hub_1_mac):
    ...
    while True:
        if keyboard.is_pressed("w"):  # Vorwärts
            await send_command(hub_1_mac, f"motor.run('C', {SPEED}); motor.run('D', {SPEED}); motor.run('E', {SPEED}); motor.run('F', {SPEED})")
        elif keyboard.is_pressed("s"):  # Rückwärts
            await send_command(hub_1_mac, f"motor.run('C', {-SPEED}); motor.run('D', {-SPEED}); motor.run('E', {-SPEED}); motor.run('F', {-SPEED})")
        elif keyboard.is_pressed("a"):  # Links drehen
            await send_command(hub_1_mac, f"motor.run('C', {-SPEED}); motor.run('E', {-SPEED}); motor.run('D', {SPEED}); motor.run('F', {SPEED})")
        elif keyboard.is_pressed("d"):  # Rechts drehen
            await send_command(hub_1_mac, f"motor.run('C', {SPEED}); motor.run('E', {SPEED}); motor.run('F', {-SPEED}); motor.run('D', {-SPEED})")
        elif keyboard.is_pressed("q"):  # Stoppen
            await send_command(hub_1_mac, "motor.stop('A'); motor.stop('E'); motor.stop('F'); motor.stop('D')")
